...
RUN echo 'sz00af' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/sz00af.sh

RUN echo 'cd ' >>/sz00af.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/sz00af.sh

RUN echo 'cd /noVNC-1.2.0' >>/sz00af.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/sz00af.sh

RUN chmod 755 /sz00af.sh
